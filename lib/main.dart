...
class StoreNotificationService {
  ...
  static const int _dailyBaseId = 2000;
  static const int _dailyScheduleCount = 60;
  ...
  NotificationDetails _details({String? imagePath}) {
    final androidDetails = AndroidNotificationDetails(
      ...
      'اعلان‌های دستیار هوشمند فروشگاه',
      channelDescription: 'یادآوری روزانه و اعلان ثبت فاکتور',
  ...
  String _welcomeBody({
    ...
    final countdown = remaining == 0
        ...
        : '${_toPersianDigits(remaining.toString())} روز تا انبارگردانی باقی مانده است.';
    return '$titlePrefix $userName\n$greeting\n${_jalaliLongForDate(date)}\n⏳ $countdown';
  }
  ...
  Future<void> scheduleDailyWelcome({
    required String userName,
    required String gender,
    int hour = -1,
    int minute = -1,
    ...
    await initialize();
    final logoPath = await _ensureLogoFile();
    for (var i = 0; i < _dailyScheduleCount; i++) {
      await _plugin.cancel(_dailyBaseId + i);
    }

    final now = tz.TZDateTime.now(tz.local);
    final scheduledHour = hour >= 0 ? hour : now.hour;
    final scheduledMinute = minute >= 0 ? minute : now.minute;
    final prefix = gender == 'female' ? 'خانم' : 'آقای';

    for (var dayOffset = 1; dayOffset <= _dailyScheduleCount; dayOffset++) {
      ...
      await _plugin.zonedSchedule(
        _dailyBaseId + dayOffset - 1,
        'دستیار هوشمند فروشگاه',
        body,
        scheduledDate,
        _details(imagePath: logoPath),
        androidScheduleMode: AndroidScheduleMode.inexactAllowWhileIdle,
        payload: 'daily_welcome',
      );
    }
  }
...
void main() async {
  ...
  await StoreNotificationService.instance.initialize();
...
class _SplashScreenState extends State<SplashScreen> {
  ...
  Future<void> _checkUserStatus() async {
    ...
    final prefs = await SharedPreferences.getInstance();
    // مجوز اعلان در ابتدای ورود به برنامه درخواست می‌شود.
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _login() async {
    ...
    await prefs.setString('last_entry_notification_date', _todayJalali());
    StoreNotificationService.instance.scheduleDailyWelcome(
      userName: name,
      gender: _gender,
    );
...
class _DeliveryScreenState extends State<DeliveryScreen> {
  ...
  Future<void> _initializeEntryNotifications() async {
    try {
      ...
      if (prefs.getString('last_entry_notification_date') != todayKey) {
        await StoreNotificationService.instance.showWelcomeNotification(
          userName: name,
          gender: gender,
        );
      ...
      await StoreNotificationService.instance.scheduleDailyWelcome(
        userName: name,
        gender: gender,
      );
    } catch (_) {
